...
아래 계획은 그대로 실행됐고, **두 종류를 유지하는 부분만 되돌렸습니다.** `bot-mineflayer` 는 archive 됐습니다. 이 문서는 결정의 기록이므로 고치지 않고, 결과를 여기에 적습니다.

계획이 mineflayer 를 남긴 근거는 메모리였습니다 — "봇 하나에 1~1.5GiB, mineflayer 는 그 열 분의 일". **그 숫자는 아무도 재지 않은 것이었습니다.** 실측하니 데스크톱(GPU 있음)에서 287MiB, 컨테이너(xvfb + llvmpipe)에서 1.6~1.7GiB 였고, 저장소 문서에는 그와 또 다른 253~347MiB 가 적혀 있었습니다. 차이는 그래픽 카드입니다: 카드가 있으면 텍스처와 청크 메시가 카드에 있고, 없으면 전부 시스템 메모리입니다. 클라이언트 설정으로는 줄지 않습니다.

메모리가 판단에서 빠지자 남은 것은 **비용과 오라클** 둘이었습니다.

- **비용**: 도구 47개를 두 번 구현하는 일. 하루치 작업이 그 비용을 그대로 보여줬습니다 — component 필드 하나를 추가할 때마다 양쪽에 쓰고 양쪽 문장을 맞췄습니다.
- **오라클**: "두 봇이 같은 말을 하는가"가 실제로 결함을 여럿 잡았습니다. 다만 그건 약한 검사입니다. 색만 입힌 채팅 한 줄을 둘 다 `Hello | world` 로 보고했고 대조 스위트는 "일치"라고 했습니다.

오라클은 **기댓값에 대고 단언하는 e2e**(`e2e/`)로 옮겼습니다. 진짜 서버와 진짜 봇을 띄우고 MCP 표면으로 불러 문장을 단언하며, CI 가 마인크래프트 버전마다 돌립니다 — 매핑이 움직였는지를 잡는 자리가 이것입니다. `dev/compare.py` 는 삭제했습니다.

mineflayer 가 못 하던 넷은 끝까지 남아 있었습니다: 다이얼로그 버튼 누르기, 스크린샷, 벽 돌아가기, 프로토콜 라이브러리보다 새로운 서버에 접속하기.

**종류를 구분하는 장치는 남겨 뒀습니다.** 카탈로그의 `kinds`, `hello` 의 `kind`, 도구별 capability 협상 모두 그대로입니다. 세 번째 종류의 봇이 생길 여지를 지우지 않기 위해서이고, 한 종류뿐인 지금은 "이 종류만 지원" 문장을 도구 설명에 붙이지 않습니다.

**그 장치가 그대로 쓰였습니다(2026-09).** 두 번째 종류 `bot-azalea`(Rust, azalea)가 들어왔습니다. 이번에는 메모리를 쟀습니다: 봇 하나에 약 8MiB, 접속 0.5초, 한 스레드로 유휴 CPU 3% 입니다. 목적은 빠른 개발 루프와 여러 봇 동시 실행이고, 화면이 필요한 도구(스크린샷, 크리에이티브 인벤토리, 책·비콘 같은 화면 조작)는 fabric 에만 둡니다. 어느 도구를 누가 답하는지는 카탈로그 `kinds` 가 말하고, e2e 는 CI 에서 두 종류를 모두 돌리며 종류에 없는 도구를 부르는 케이스는 건너뜁니다. operator 도 0.9.0 부터 `kind: azalea` 파드를 만듭니다.
